...
var g = svg.append("g")
  ...
  const graph = g.selectAll(".bar")
    ...
    .attr("y", function(d) { return y(!isNaN(d[type])) ? y(d[type]) : 0 } )
    .attr("width", x.bandwidth())
    .attr("height", function(d) { return height - (y(!isNaN(d[type])) ? y(d[type]) : 0) })
    .on(["focus","hover"], function(d) {return tip.text((y(!isNaN(d[type])) ? y(d[type]) : "Unknown")).style("visibility", "visible").style("top", y((y(!isNaN(d[type])) ? y(d[type]) : 0)) - 13+ 'px' ).style("left", x(d.date) + x.bandwidth() - 12 + 'px')})
    ...
    .attr("class", "labelled")
    .text(function(d) { 
        return d[type] !== "NaN" ? d[type] : "Unknown";
    });
